  ...
  enchantPlant(potion, plant, uiManager){
    ...
    if (this.inventory[potion] > 0){
      if (this.garden[plant] > 0){
        ...
        if (!this.enchanted){
          this.enchanted++;
          this.enchantedGarden = setInterval(this.enchantedGardenWorker.bind(this), 10000);
        }
      } else {
  ...
  enchantedGardenWorker() {
    ...
    for (const [key, amountPlant] of Object.entries(this.garden)) {
      if ("gives" in Plants[key]) {
        ...
        let finalAmount;
        if (key == "enchanted flower") {finalAmount = 1; } 
  ...
  gather(uiManager) {
    if (this.gatherFlag == true) {
      ...
      Object.keys(Gatherables).forEach((key) => {
        if (Gatherables[key].discovered || Gatherables[key].auto){
          ...
          if (num > 0){
            if (key in this.inventory){
              ...
              if (!Gatherables[key].discovered) Gatherables[key].discovered = true;
